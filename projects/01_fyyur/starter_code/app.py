...
def venues():
  ...
  venues_by_area=db.session.query(Venue.city,Venue.state).group_by(Venue.state,Venue.city).all()
  data=[]
...
def show_venue(venue_id):
  ...
  now=datetime.now()
  for show in shows:
    artist_show_id=show.artist_id
    artist=Artist.query.get(artist_show_id)
    show_info ={
...
def show_artist(artist_id):
  ...
  now=datetime.now()
  for show in shows:
    venue_show_id=show.venue_id
    venue=Venue.query.get(venue_show_id)
    show_info ={
...
def shows():
  ...
  # TODO: replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  Shows=db.session.query(Show.id,Show.upcoming,Show.venue_id,Show.artist_id,Show.start_time).all()
  data=[]
  for show in Shows:
    venue=Venue.query.get(show[2])
    artist=Artist.query.get(show[3])
    if(show[1]):
      data.append({
      "venue_id":show[2],
      "venue_name":venue.name,
